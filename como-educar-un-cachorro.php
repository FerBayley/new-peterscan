<!DOCTYPE html>
<html lang="es">
<head>
	<meta charset="UTF-8">
	<title>PetersCan - Como educar a un cachorro</title>

	<?php include("includes/data.php"); ?>
</head>
<body>

	<section class="Main">
		
		<?php include("includes/menu.php"); ?>

		<section class="Contenedor-adiestramiento">
			<div class="Logo-interno">
				<a href="index.php">
					<img src="img/logo-interno.jpg" alt="PeterScan">
				</a>
			</div> <!-- End of Logo-interno -->
		</section> <!-- End of Contenedor-adiestramiento -->

		<section class="Descripcion-seccion">
			<p>SECCION: Como educar un cachorro</p>
		</section> <!-- End of Descripcion-seccion -->

		<section class="Contenedor-contenido">
			<aside>
				<ul>
					<li><a href="cachorros.php">ADQUISICION</a></li>
					<li><span>COMO EDUCAR A UN CACHORRO SEGUN SUS CARACTERISTICAS Y NECESIDADES</span></li>
					<li><a href="desde-el-primer-dia.php">QUE ENSE&Ntilde;ARLES DESDE EL PRIMER DIA EN NUESTRO HOGAR</a></li>
					<li><a href="planificacion-y-desarrollo-del-adiestramiento.php">PLANIFICACION DEL ADIESTRAMEINTO</a></li>
					<li><a href="adiestramiento-para-cachorros.php">ADIESTRAMIENTO</a></li>
				</ul>
			</aside> <!-- End of aside -->

			<div class="Contenido">
				<article>
					<h2>COMO EDUCAR A UN CACHORRO TENIENDO EN CUENTA SUS CARACTERISTICAS 
					    Y NECESIDADES
					</h2>

					<p>
						Es fundamental tener informaci&oacute;n sobre el estilo de vida 
						que requiere un perro y cu&aacute;les son las 
						caracter&iacute;sticas de cada raza, para poder cubrir sus 
						necesidades vitales y comprender el comportamiento normal 
						del cachorro en las distintas etapas de su desarrollo.
					</p>					

					<p>
						Para que crezca con confianza es conveniente estimularlo con 
						caricias f&iacute;sicas y verbales, protegi&eacute;ndolo ante 
						cualquier situaci&oacute;n que le genere miedo.
					</p>	

					<p>
						Es conveniente comenzar a educar al nuevo integrante de la familia 
						a partir del d&iacute;a mismo de su llegada. Educarlo desde el 
						primer instante, por m&aacute;s que parezca duro y nos d&eacute; 
						pena, es lo mejor para el cachorro. Es muy lindo llevar al 
						cachorro a nuestra cama el primer d6iacute;a, pero despu&eacute;s 
						ser&aacute; muy dif&iacute;cil lograr que duerma solo. 
						Es triste sentirlo llorar, pero ser&aacute; m&aacute;s lamentable 
						escucharlo aullar cuando sea grande y no quiera estar solo.
					</p>	

					<p>
						En este periodo hay que tener en cuenta la tensi&oacute;n 
						emocional (stress) que le genera a un cachorro el cambio de 
						h&aacute;bitat. Viene de vivir con su mam&aacute; y hermanos, en 
						un lugar cuyos sonidos y especialmente olores le son familiares y 
						conoce &#34;desde siempre&#34;, a otro lugar, sin referencias 
						olfativas ni sonoras, donde no encuentra el calor de la mam&aacute; 
						y los hermanos, que deben ser reemplazados por la seguridad que 
						nosotros le hagamos sentir.
					</p>	

					<p>
						Deberemos ayudarlo a superar los miedos (a la oscuridad, al 
						silencio nocturno, a dormir solo), a hacer sus necesidades en 
						el lugar elegido para este fin, a respetar el lugar que 
						deber&aacute; ocupar en la jerarqu&iacute;a familiar, a quedarse 
						solo por peque&ntilde;os periodos, a jugar en los lugares 
						apropiados y a aceptar las primeras reglas de convivencia familiar. 
						Desde el principio debemos ense&ntilde;arle las costumbres que 
						regir&aacute;n en la relaci&oacute;n con su nueva familia, tal 
						como, aunque m&aacute;s lentamente, lo hacemos cuando llega a la 
						familia un &#34;cachorro&#34; humano.
					</p>

					<p>
						Es necesario ser claros en la comunicaci&oacute;n con nuestro 
						perro. Ellos captan perfectamente la no verbalidad de sus 
						due&ntilde;os. Los gestos, la mirada, la postura corporal, los 
						tonos de voz, tienen m&aacute;s peso que las palabras. Por 
						ejemplo, si le ordenamos al cachorro que no muerda las manos con 
						una sonrisa o tono de voz suave, es casi seguro que no 
						obedecer&aacute; la orden, debido a que para &eacute;l la 
						sonrisa est&aacute; asociada a momentos de juego o aprobacin, y 
						el tono de voz suave lo refuerza. As&iacute;, la orden verbal 
						pierde fuerza
					</p>

					<p>
						Es conveniente que el grupo familiar tenga una idea formada sobre 
						las normas que desea ense&ntilde;arle al cachorro, para no 
						confundirlo. Para eso, lo ideal es haberlo conversado antes de 
						llevar el animal a su nuevo hogar y cumplir los roles que cada uno 
						haya asumido; si alimentarlo, pasearlo, ba&ntildE;arlo, jugar con 
						&eacute;l, impartirle normas de conducta, ser&aacute;n tareas 
						repartidas o compartidas, o cada uno se ocupar&aacute; de 
						determinado rol.
					</p>

					<p>
						Mi tarea consiste en asesorar sobre las conductas de los cachorros 
						que viven en un ambiente humano. El comportamiento de los 
						perros dentro de una familia es producto del bagaje 
						gen&eacute;tico que traen consigo, sumado al gui&oacute;n familiar 
						que trasmiten sus due&ntilde;os desde su llegada al nuevo hogar. 
						Esta fusi&oacute;n, que en muchos casos confunde al cachorro, 
						sobre todo si vive en una familia numerosa, es lo que tenemos 
						que tener en cuenta a la hora de educarlo y adiestrarlo.
					</p>

					<div class="Accesos-facebook">
						<a href="https://www.facebook.com/media/set/?set=a.826809777377672.1073741829.193071660751490&type=3&uploaded=14" target="_blank">
							<img src="img/ver-mas.jpg" alt="ver mas">
						</a>
					</div> <!-- End of Accesos-facebook -->
				</article>
			</div> <!-- End of Contenido -->
		</section> <!-- End of Contenedor-contenido -->

		<?php include("includes/influencia.php"); ?>

		<?php include("includes/redes.php"); ?>

		<?php include("includes/footer.php"); ?>
		
	</section> <!-- End of Main -->
	
</body>
</html>