<!DOCTYPE html>
<html lang="es">
<head>
	<meta charset="UTF-8">
	<title>Peterscan - Perros con poder</title>

	<?php include("includes/data.php"); ?>
</head>
<body>

	<section class="Main">
		
		<?php include("includes/menu.php"); ?>

		<section class="Contenedor-adiestramiento">
			<div class="Logo-interno">
				<a href="index.php">
					<img src="img/logo-interno.jpg" alt="PeterScan">
				</a>
			</div> <!-- End of Logo-interno -->
		</section> <!-- End of Contenedor-adiestramiento -->

		<section class="Descripcion-seccion">
			<p>SECCION: Perros con poder</p>
		</section> <!-- End of Descripcion-seccion -->

		<section class="Contenedor-contenido">
			<aside>
				<img src="img/banners/perros-con-poder.jpg" alt="Perros con poder">
			</aside> <!-- End of aside -->

			<div class="Contenido">
				<article>
					<h2>
						COMPORTAMIENTOS DE PERROS HUMANIZADOS QUE TIENEN PODER SOBRE 
						SUS DUE&Ntilde;OS
					</h2>

					<p>
						Destruyen objetos, ropa y plantas y se asignan un sill&oacute;n o 
						una cama para dormir o simplemente para descansar o estar 
						all&iacute; cuando lo desean. Para sus propietarios es tan 
						agotador modificar estas conductas de sus perros, que terminan 
						aceptando y en muchos casos justificando esta costumbre de sus 
						queridos perros.
					</p>
					
					<p>
						Gru&ntilde;en si los retan cuando muerden los zapatos, los 
						pantalones, las manos, los brazos, o si se les quiere sacar 
						algo que tomaron como propio.
					</p>
					
					<p>
						Se vuelven posesivos y no permiten que se acerquen a sus 
						propietarios con quien han establecido un v&iacute;nculo muy 
						estrecho. Su comportamiento es decididamente hostil e 
						intimidatorio si se acercan a su due&ntilde;o, o intentan sacarlos 
						del lugar en el que est&aacute;n.
					</p>

					<p>
						Desayunar, almorzar merendar o cenar se puede convertir en una 
						pesadilla, imposible relajarse para hablar mientras se come, o 
						se mira una pel&iacute;cula y mucho menos, hacer una sobremesa el tiempo 
						que se desee. Hay que estar permanentemente atentos para impedir 
						que el perro no se robe comida de la mesa.
					</p>

					<p>
						Hablar por tel&eacute;fono puede llegar a ser una misi&oacute;n 
						imposible, el perro comienza a ladrar apenas escucha el sonido 
						del tel&eacute;fono impidiendo que se pueda mantener una 
						comunicaci&oacute;n normal. Finalmente su propietario termina 
						hablando a los gritos en medio de los ladridos, o se escapa y 
						se recluye en otro ambiente en el cual no llegue el perro ni 
						sus ladridos.
					</p>
				</article>

				<p>
					Salir de nuestra propia casa puede ser una verdadera odisea, en el 
					preciso instante que decidimos salir, comienza a ladrar y se pone en 
					la puerta impidi&eacute;ndonos el paso. Intentamos todos los trucos 
					sabidos, ponemos en pr&aacute;ctica lo que dice el &#34;Encantador 
					de Perros&#34; sobre este tema, pero &eacute;l continua ladrando y 
					empecinado en no dejarnos salir, a menos sin &eacute;l. En un acto 
					supremo de audacia nos abalanzamos sobre la puerta e intentamos 
					introducir en el primer intento la llave en la cerradura para 
					abrirla y escapar de nuestro hogar, para tomar distancia por unas 
					horas de nuestro mejor amigo.
				</p>

				<p>
					Para las personas que han vivido solas mucho tiempo con su querido 
					perro, e incorporan a su vida una pareja, puede ser el inicio de un 
					conflicto entre el perro y el nuevo integrante. Estos perros por lo 
					general son muy celosos, les cuesta compartir en lugar que tienen 
					adjudicado en la casa y en el afecto de sus due&ntilde;os. Las 
					conductas que emplean los perros en estos casos son variadas, pero 
					las que m&aacute;s se observan son las siguientes: no acepta caricias 
					f&iacute;sicas ni verbales del nuevo integrante, le orinan los
					pantalones y los zapatos, destruyen, o esconden su ropa intima, se 
					suben a la cama en momentos inoportunos, piden salir a la calle para 
					hacer pipi, o popo m&aacute;s veces de lo que necesitan, corren en 
					circulo invitando a su due&ntilde;o a jugar, nunca, al intruso/a, 
					se esconden debajo de los sillones o muebles para desde all&iacute; 
					gru&ntilde;ir en el caso que lo quieran sacar, o hacen sus necesidades 
					en la cama del propietario.
				</p>

				<p>
					Los perros humanizados acostumbrados a tener un lugar de privilegio 
					en la vida de sus due&ntilde;os, tienen una gama amplia de conductas 
					para llamar la atenci&oacute;n. Estos son algunos ejemplos ilustrativos 
					que muestran cuando los perros tienen poder sobre sus due&ntilde;os. 
					Este poder, no nace con los perros, lo van adquiriendo a medida que 
					crecen y son sus propietarios quienes se lo adjudican.
				</p>
				<div class="Accesos-facebook">
					<a href="https://www.facebook.com/media/set/?set=a.826874537371196.1073741835.193071660751490&type=3&uploaded=16" target="_blank">
						<img src="img/ver-mas.jpg" alt="ver mas">
					</a>
				</div> <!-- End of Accesos-facebook -->
			</div> <!-- End of Contenido -->
		</section> <!-- End of Contenedor-contenido -->

		<?php include("includes/influencia.php"); ?>

		<?php include("includes/redes.php"); ?>

		<?php include("includes/footer.php"); ?>
		
	</section> <!-- End of Main -->
	
</body>
</html>