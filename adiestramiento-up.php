<!DOCTYPE html>
<html lang="es">
<head>
	<meta charset="UTF-8">
	<title>PetersCan - Adiestramiento UP</title>

	<?php include("includes/data.php"); ?>
</head>
<body>

	<section class="Main">
		
		<?php include("includes/menu.php"); ?>

		<section class="Contenedor-adiestramiento">
			<div class="Logo-interno">
				<a href="index.php">
					<img src="img/logo-interno.jpg" alt="PeterScan">
				</a>
			</div> <!-- End of Logo-interno -->
		</section> <!-- End of Contenedor-adiestramiento -->

		<section class="Descripcion-seccion">
			<p>SECCION: Adiestramiento UP</p>
		</section> <!-- End of Descripcion-seccion -->

		<section class="Contenedor-contenido">
			<aside>
				<img src="img/banners/adiestramiento-up.jpg" alt="Adiestramiento UP">
			</aside> <!-- End of aside -->

			<div class="Contenido">
				<article>
					<h2>UN CONCEPTO DIFERENTE EN EL ADIESTRAMIENTO DE PERROS HUMANIZADOS</h2>
					<p>
						Muchos de los perros humanizados que se observan en la actualidad, 
						ocupan un lugar muy importante en la vida de las personas, porque 
						representan para sus due&ntilde;os, desde lo afectivo, como si 
						fueran un hijo, un nieto, un sobrino, un hermano o un amigo del alma.
					</p>

					<p>
						Este lugar afectivo de los perros en la vida de las personas 
						est&aacute; relacionado con diversas causas, por ejemplo: 
						situaciones de estr&eacute;s, separaciones conyugales, partida de 
						los hijos del hogar, deseos de tener hijos y no poder concebirlos, 
						el miedo a la soledad, la muerte de un ser querido, ataques de 
						p&aacute;nico, cuadros depresivos y problemas motrices que no 
						permiten moverse y desplazarse con autonom&iacute;a entre otros 
						motivos.
					</p>

					<p>
						Cuando en el a&ntilde;o 1979 comenc&eacute; mi actividad como 
						instructor canino, las personas ten&iacute;an tambi&eacute;n una 
						relaci&oacute;n muy afectiva con sus perros, pero sin que esto 
						alterara el lugar del perro en la familia.La relaci&oacute;n entre 
						las personas y sus perros era clara, ellos reci&iacute;an mucho 
						cari&ntilde;o y cuidados pero nadie en la familia ten&iacute;a 
						dudas que el perro era la mascota del hogar y de acuerdo a la 
						raza, el guardi&aacute;n de la casa.
					</p>

					<p>
						Los perros ten&iacute;an nombres afectivos diminutivos de series de 
						TV o de dibujos animados ( Pluto, Colita, Lassie, Layka, 
						Zandok&aacute;n, Atila ). Los nombres eran atributos que convocaban 
						al juego, a la infancia, a la aventura, o a la protecci&oacute;n 
						que ellos brindaban).
					</p>

					<p>
						Finalizada la d&eacute;cada del 80 y a medida que trascurría la del 
						90 era habitual escuchar en la calle llamar a los perros 
						afectivamente con nombres de personas o dioses mitol&oacute;gicos 
						( Jos&eacute;, Luna, Felipe, Juana, Santos, Lola, Theo, Zeus, 
						Trit&oacute;n, Apolo, H&eacute;rcules, Ulises) La relaci&oacute;n 
						de las personas con sus perros iba cambiando progresivamente, el 
						v&iacute;nculo que manten&iacute;an con ellos era cada vez 
						m&aacute;s afectivo y acorde a la transformaci&oacute;n que se 
						iba operando en esos a&ntilde;os.
					</p>

					<p>
						El trabajo de adiestramiento de cientos de perros a lo largo de 
						m&aacute;s de 30 a&ntilde;os de trabajo en el tema y la 
						observaci&oacute;n de las conductas de las personas con sus perros 
						me permiti&oacute; crear un adiestramiento diferente, adaptado a 
						esta nueva realidad.
						El Adiestramiento UP brinda a las personas estrechamente vinculadas 
						emocionalmente a sus perros las herramientas necesarias para que 
						puedan ponerles l&iacute;mites a sus perros sin sentirse culpables, 
						o que pierden el v&iacute;nculo afectivo con ellos.
					</p>
				</article>
				<div class="Accesos-facebook">
					<a href="https://www.facebook.com/media/set/?set=a.826867727371877.1073741833.193071660751490&type=3&uploaded=10" target="_blank">
						<img src="img/ver-mas.jpg" alt="ver mas">
					</a>
				</div> <!-- End of Accesos-facebook -->

			</div> <!-- End of Contenido -->
		</section> <!-- End of Contenedor-contenido -->

		<?php include("includes/influencia.php"); ?>

		<?php include("includes/redes.php"); ?>

		<?php include("includes/footer.php"); ?>
		
	</section> <!-- End of Main -->
	
</body>
</html>